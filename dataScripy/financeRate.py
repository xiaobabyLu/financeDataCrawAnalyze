...
提取标签属性值:  https://blog.csdn.net/liao392781/article/details/80181088
'''
...
if __name__ == '__main__':
    ...
    for tdall in trs:
        #解析网页可以获取到 当 len(tdall)是每一行我们需要的数据，其他数据我们并不需要，是脏数据，可以通过这个进行过滤
        if len(tdall) ==4:
            tds = tdall.find_all('td')
            for td in tds:
                vlaue = td.find(name= 'div',attrs={"class":"tishi"})
                #<>.text获取标签中的值
                print(vlaue.text)
